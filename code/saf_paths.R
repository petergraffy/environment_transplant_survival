resolve_saf_dir <- function(required = TRUE) {
  env_candidates <- c(
...
get_saf_paths <- function(required = TRUE) {
  saf_dir <- resolve_saf_dir(required = required)
...
assert_saf_files <- function(paths, include_stathist = FALSE) {
  required_files <- c(
    ...
    file.path(paths$supp_dir, "canzip2506.sas7bdat")
  )

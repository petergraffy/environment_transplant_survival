...
theme_subgroup_forest <- function(base_size = 26) {
  theme_minimal(base_size = base_size) +
    theme(
      ...
      strip.background = element_rect(fill = "white", color = "grey30", linewidth = 0.7),
...
plot_one_model <- function(dat, model_value, title_text) {
  plot_dat <- dat %>%
    filter(model == model_value, is.na(error), !(subgroup_label == "Race" & subgroup_level == "Other/Unknown")) %>%
    mutate(
  ...
  ggplot(plot_dat, aes(x = hazard_ratio, y = subgroup_level, color = pollutant)) +
    geom_vline(xintercept = 1, linewidth = 0.4, color = "grey30") +
    geom_errorbar(
    ...
    geom_point(size = 3.3, shape = 16, position = position_dodge(width = dodge_width)) +
    geom_text(
    ...
    scale_x_log10(
      breaks = c(0.6, 1.0, 1.5, 2.0, 3.0),
      labels = label_number(accuracy = 0.01),
      expand = expansion(mult = c(0.01, 0.01))
    ) +
    coord_cartesian(xlim = c(0.50, 3.5), clip = "off") +
...
png_path <- file.path(fig_dir, "prior_year_and_timevarying_pollution_subgroup_forest.png")
pdf_path <- file.path(fig_dir, "prior_year_and_timevarying_pollution_subgroup_forest.pdf")
baseline_png_path <- file.path(fig_dir, "prior_year_pollution_subgroup_forest.png")
baseline_pdf_path <- file.path(fig_dir, "prior_year_pollution_subgroup_forest.pdf")
timevarying_png_path <- file.path(fig_dir, "timevarying_pollution_subgroup_forest.png")
timevarying_pdf_path <- file.path(fig_dir, "timevarying_pollution_subgroup_forest.pdf")
ggsave(png_path, combined, width = 34, height = 34, dpi = 320, bg = "white")
ggsave(pdf_path, combined, width = 34, height = 34, device = cairo_pdf, bg = "white")
ggsave(baseline_png_path, p_baseline, width = 34, height = 17, dpi = 320, bg = "white")
ggsave(baseline_pdf_path, p_baseline, width = 34, height = 17, device = cairo_pdf, bg = "white")
ggsave(timevarying_png_path, p_tv, width = 34, height = 17, dpi = 320, bg = "white")
ggsave(timevarying_pdf_path, p_tv, width = 34, height = 17, device = cairo_pdf, bg = "white")

write_csv(
  tibble(
    figure = c(
      "prior_year_and_timevarying_pollution_subgroup_forest",
      "prior_year_and_timevarying_pollution_subgroup_forest",
      "prior_year_pollution_subgroup_forest",
      "prior_year_pollution_subgroup_forest",
      "timevarying_pollution_subgroup_forest",
      "timevarying_pollution_subgroup_forest"
    ),
    path = normalizePath(
      c(
        ...
        baseline_png_path,
        baseline_pdf_path,
        timevarying_png_path,
        timevarying_pdf_path
      ),
      winslash = "/",
      mustWork = FALSE
    )
  ),
  file.path(fig_dir, "prior_year_and_timevarying_pollution_subgroup_forest_manifest.csv")
)

message("Wrote prior-year and time-varying subgroup forest plot to ", normalizePath(fig_dir, winslash = "/"))

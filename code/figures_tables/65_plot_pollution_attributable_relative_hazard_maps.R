...
cox_path <- file.path("output", "timevarying_pollution_cox_svi", "timevarying_pollution_cox_svi_results.csv")
...
map_values <- pollution_means %>%
  ...
  mutate(
    ...
    excess_relative_hazard = pmax(relative_hazard - 1, 0),
    pollutant_panel = factor(exposure, levels = names(pollutant_specs)),
    organ = factor(organ, levels = c("Kidney", "Liver", "Heart", "Lung")),
    organ_panel = factor(organ_panel_labels[as.character(organ)], levels = organ_panel_labels)
  )
...
map_summary <- map_values %>%
  ...
  summarise(
    ...
    min_excess_relative_hazard = min(excess_relative_hazard, na.rm = TRUE),
    p99_excess_relative_hazard = as.numeric(quantile(excess_relative_hazard, 0.99, na.rm = TRUE)),
    max_excess_relative_hazard = max(excess_relative_hazard, na.rm = TRUE),
    .groups = "drop"
  )
...
plot_dat <- merge(
  zcta,
  map_values %>% select(zip, exposure, pollutant_panel, organ_panel, relative_hazard, excess_relative_hazard),
  by = "zip",
  all.x = FALSE
)
...
panel_limits <- plot_dat %>%
  st_drop_geometry() %>%
  group_by(exposure, organ_panel) %>%
  summarise(
    lo = 0,
    hi = as.numeric(quantile(excess_relative_hazard, 0.99, na.rm = TRUE)),
    .groups = "drop"
  )
...
legend_breaks <- pretty(c(global_limits$lo, global_limits$hi), n = 8)

plot_dat <- plot_dat %>%
  mutate(
    excess_relative_hazard_winsor = pmin(pmax(excess_relative_hazard, global_limits$lo), global_limits$hi)
  )

write_csv(
  tibble(
    scale = "shared_excess_relative_hazard_0_to_panel_p99_rounded",
    lo = global_limits$lo,
    hi = global_limits$hi,
...
theme_map <- function() {
  theme_void(base_size = 11.5) +
    theme(
      ...
      strip.text = element_text(face = "bold", size = 14, color = "grey10", margin = margin(8, 8, 8, 8)),
...
write_csv(
  tibble(
    ...
    description = "Model-implied PM2.5 and NO2 excess relative hazard by ZCTA versus national median pollutant concentration, with values below the national median-implied hazard set to 0."
  ),

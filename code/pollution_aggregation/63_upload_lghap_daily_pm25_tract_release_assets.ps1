param(
  ...
  [string]$Tag = "lghap-pm25-tract-daily-cook-county-il-v1",
  [string]$Target = "main",
  [string]$AssetDir = "data/release/lghap_pm25_tract_daily_cook_county_il_csv",
...
$headers = @{
  ...
  "User-Agent" = "environment-transplant-survival-lghap-pm25-tract-upload"
}
...
try {
  ...
  $body = @{
    ...
    name = "LGHAP daily PM2.5 Cook County census tract CSV assets v1"
    body = @"
Daily LGHAP PM2.5 exposures aggregated to 2020 Cook County, Illinois census tract polygons, 2005-2021.

Assets:
- ``lghap_pm25_tract_daily_YYYY.csv.gz``: yearly daily tract PM2.5 values, one row per tract-date.
- ``lghap_pm25_tract_daily_csv_manifest.csv``: row counts, date coverage, completeness, sizes, and SHA-256 checksums.
- ``lghap_pm25_tract_daily_qc_all_months.csv``: monthly completeness and distribution QC.
...
The repository contains the reproducible daily NetCDF-to-census-tract aggregation and release packaging scripts.
"@
    draft = $false
    prerelease = $false
  }
...
$assetFiles = Get-ChildItem -LiteralPath $AssetDir -File |
  Where-Object { $_.Name -match "(\.csv\.gz|\.csv|\.md)$" } |
  Sort-Object Name
